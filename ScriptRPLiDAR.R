...
#without remove text
ggarrange (plot5, plot6,labels = c("A","B","C"),ncol=1, nrow=3)
...
#no remove text
ggarrange (barplot5, barplot6,labels = c("A","B","C"),ncol=1, nrow=3)


plot1 <- ggplot(Test.Api1.300, aes(x = Angule)) + geom_bar(aes(fill = Distance),
      position = 'dodge') + theme(axis.text.x = element_text(angle = 90), color = Jenis)
...
ggarrange (scplot1, scplot2, scplot3+rremove("x.text"),labels = c("A","B","C"),ncol=2, nrow=2)
